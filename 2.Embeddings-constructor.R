...
# =====================================
tpm_mat <- read.csv("exports/final-supplementary-data/tpm.csv", row.names=1, check.names=FALSE)
...
cat("\n--- Checking 'molecular_subtype' column ---\n")
if("molecular_subtype" %in% colnames(meta_df)){
  cat("Unique molecular molecular_subtypes:\n")
  print(table(meta_df$molecular_subtype, useNA = "ifany"))
} else {
  cat("⚠ 'molecular_subtype' column not found!\n")
  cat("Available columns:", paste(colnames(meta_df), collapse=", "), "\n")
}
...
required_cols <- c("grade", "molecular_subtype")
...
if(nrow(meta_df_clean) == 0){
  stop("No samples remaining after removing NAs from grade/molecular_subtype!")
}
...
g_h <- getGmt("og-supplementary-data/c2.all.v2025.1.Hs.symbols.gmt")
g_c2 <- getGmt("og-supplementary-data/h.all.v2025.1.Hs.symbols.gmt")
...
for(s in names(stratified_results)){
  ...
  write.csv(
    phate_obj$embedding,
    paste0("exports/embeddings/embeddings_", s, ".csv"),
    row.names = TRUE
  )
  
  cat("  Saved:", paste0("exports/embeddings_", s, ".csv"), "\n")
}
...
# =====================================
cat("\n=== SAVING RESULTS ===\n")
dir.create("exports/embeddings/gsva_xcell", recursive = TRUE)
dir.create("exports/embeddings/xcell", recursive = TRUE)
saveRDS(stratified_results, "exports/embeddings/gsva_xcell/stratified_gsva_xcell.rds")
saveRDS(cell_fraction_results, "exports/embeddings/xcell/stratified_xcell_only.rds")
saveRDS(phate_embeddings, "exports/embeddings/phate_embeddings.rds")
